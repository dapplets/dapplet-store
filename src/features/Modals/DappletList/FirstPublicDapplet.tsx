...
const FirstPublicDapplet = ({ settings }: FirstPublicDappletProps) => {
  ...
  return (
    <ModalCard>
      ...
      <Body>
        <Message>
          You will be able to use the dapplets in this list through the Extension on all computers
          where you are logged in. You will also be able to share this list. Changing this list
          requires a blockchain transaction signature and some cost. If you don't want to spend, you
          can add a dapplet to the Extension List, changes to which do not require a transaction
          signature, but which is local.
        </Message>
        <Controls>
