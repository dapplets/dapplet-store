...
const SidePanel = ({
  ...
  const pushSelectedDappletsList = async () => {
    ...
    if (shouldClearList) links.push({ prev: PUBLIC_LIST.HEADER, next: PUBLIC_LIST.TAIL })
        ...
        if (!shouldClearList) {
          ...
          if (isInitialEvent) {
            ...
            const exPrev = listingEventsReversedCopy.find((ev, evIndex) => {
              ...
              return evIndex > indexFromBehind && ev.type === DappletsListItemTypes.Default
            })

            const exNext = listingEvents.find(
              (ev, evIndex) => evIndex > i && ev.type === DappletsListItemTypes.Default
            )
    ...
    const deduplicatedLinks = links.filter(
      (currentLink, index, links) =>
        ...
        links.findIndex(
          (linkToCompareTo) =>
            linkToCompareTo.prev === currentLink.prev && linkToCompareTo.next === currentLink.next
        )
    )
  ...
  return (
    <Wrapper className={className}>
      ...
      <SideNav selectedDappletsList={selectedDappletsList} onPush={pushSelectedDappletsList} />

      <Footer>
        <FooterLinks>
          <a href="https://dapplets.org/under-construction.html">Terms & Conditions</a>
          <a href="https://dapplets.org/under-construction.html">Privacy Policy</a>
