...
import { removeDappletItem, updateDappletItem } from '../../../lib/updateDappletItem'
...
const DappletList = ({
  ...
  const editSelectedDappletsList = useMemo(
    () => (item: IDapplet) => {
      ...
      const indexSelectedDapplet = nowDappletsList.findIndex((d) => d.name === item.name)
      ...
      if (indexSelectedDapplet >= 0) {
        ...
        switch (type) {
          ...
          case DappletsListItemTypes.Default: {
            ...
            nowDappletsList = updateMyListing(nowDappletsList, removing, indexSelectedDapplet)
            break
          }
          ...
          case DappletsListItemTypes.Removing: {
            ...
            nowDappletsList = updateMyListing(nowDappletsList, adding, indexSelectedDapplet)
            break
          }

          case DappletsListItemTypes.Adding: {
            nowDappletsList = removeDappletItem(nowDappletsList, indexSelectedDapplet)
            break
          }
  ...
  }, [searchQuery, addressFilter, isTrustedSort, selectedList, sortType, trigger, ref])
        ...
        <div /* style={{ padding: "0 15px" }} */>
          ...
          <MainContentWrapper>
            ...
            {address !== addressFilter &&
              !isNotDapplet &&
              !trustedUsers.map((user) => user.hex).includes(addressFilter || '') && (
                <CheckboxWrapper
