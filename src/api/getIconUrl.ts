...
const getIconUrl = async (storageRef: StorageRef) => {
  ...
  for (const uri of storageRef.uris) {
    ...
    if (protocol === 'bzz') {
      promises.push(
        getAndCheckHash(BZZ_ENDPOINT + '/bzz/' + reference, controller, storageRef.hash)
      )
    } else if (protocol === 'ipfs') {
      promises.push(
        getAndCheckHash(IPFS_ENDPOINT + '/ipfs/' + reference, controller, storageRef.hash)
      )
    } else if (protocol === 'sia') {
      promises.push(getAndCheckHash(SIA_ENDPOINT + '/' + reference, controller, storageRef.hash))
    } else {
  ...
  if (storageRef.hash) {
    ...
    if (hash.match(/^0*$/) === null)
      promises.push(getAndCheckHash(S3_ENDPOINT + '/' + hash, controller, storageRef.hash))
  }
