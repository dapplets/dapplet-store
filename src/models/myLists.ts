...
const effects = (dispatch: any) => ({
  ...
  async getMyListing({ address, provider, dappletsNames }: { address: string, provider: any, dappletsNames: { [name: number]: string } }) {
    ...
    const listing: MyListElement[] = req.map((id: BigNumber) => {
      const dapp = {
        id: id.toNumber(),
    ...
    const addingListing: MyListElement[] = Object.values(localListing).filter(({ type }) => type === DappletsListItemTypes.Adding)
