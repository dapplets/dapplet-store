...
const HEADER_MESSAGES = {
  success: (
    <React.Fragment>
      <Installed />
      <h6 className="notification-custom-title finished">
...
export const Notification: FC<SuccessToastProps> = ({
  ...
  return (
    ...
    >
      <header className={cn("notification-custom-header")}>
        {HEADER_MESSAGES[type]}
      </header>
      <p className={cn("notification-custom-text")}>
        ...
        {isError && messageErrror && messageErrror.length > 0 && (
          <span className="notification-custom-text-error">
            {messageErrror}
          </span>
        )}
      </p>
